...
let globalWords = []; //array keeping track of lines as they are updated
...
function getWordAtLocation(aCanvasX, aCanvasY) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    (aCanvasY > words[i].y -10 - wordHeight && aCanvasY < words[i].y+10)) {
...
function drawCanvas() {
  ...
  let chordFillColor2 = 'orange'
  let chordStrokeColor2 = 'orange'
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (data.chord) {
      ...
      if (transposed == 0){
        ...
        context.fillText(find, data.x, data.y-20)
        context.strokeText(find, data.x, data.y-20)
      }
      else{
        context.fillStyle = chordFillColor2
        context.strokeStyle = chordStrokeColor2
        context.fillText(find, data.x, data.y-20)
        context.strokeText(find, data.x, data.y-20)
      }
    }
...
function parseChordProFormat(chordProLinesArray) {
  ...
  for (let i = 0; i < chordProLinesArray.length; i++) {
    globalWords[i] = (chordProLinesArray[i]); //puts words in the gloabal array as well
    let line = chordProLinesArray[i]
    textDiv.innerHTML = textDiv.innerHTML + `<p> ${line}</p>` //adds text on bottom
  ...
  if (userText && userText !== '') {
    ...
    $.post("fetchSong", userRequestJSON, function(data, status) {
      ...
      if (responseObj.songLines) {
        ...
        parseChordProFormat(responseObj.songLines);
      }
    })
  }
}
...
function handleRefreshButton() {
  ...
  //for loops go throught all of canvas
  for(let i = 0; i < canvas.height + 400; i += 20){
    let on = false;
    let create = ""
    for(let j = 0; j < canvas.width + 100; j += 10) {
      let place = getWordAtLocation(j, i) //finds words on the cancas and saves it to "place"
      if(sets.has(place) === false && place != null) {
        on = true
        sets.add(place)
        create += place.word + " " //adds lyrics and spaces
      }
    }
    if(on) {
      if (globalWords.length != 0 && count == 0){
        count++
        globalWords = []
      }
      globalWords.push(create) //creates new array
    }
  }
  for (let k = 0; k < globalWords.length; k++) {
    textDiv.innerHTML = textDiv.innerHTML + `<p> ${globalWords[k]}</p>` //refreshed canvas
  }
}
...
function handleSaveAsButton() {
  ...
  let newString = ""; //empy string to be filled
  if(userText && userText != '') {
    $('#userTextField').val('')
    //creates file to save
    for(let i = 0; i < globalWords.length; i++) {
      newString += globalWords[i] + "\n";
    }
    let newObj = {wrds: newString, text: userText}
    newObj = JSON.stringify(newObj); //makes JSON string
    //send
    $.post("saveSong", newObj, function(data, status) {
      console.log("data: " + data)
      console.log("typeof: " + typeof data)
    })
  }
