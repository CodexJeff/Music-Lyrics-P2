...
http.createServer(function(request, response) {
  ...
  request.on("end", function() {
    ...
    if (request.method === "POST" && urlObj.pathname === "/fetchSong") {
      ...
      let dataSongLines = JSON.parse(receivedData); //gets data
      let newSaveAsFilePath = "songs/" + dataSongLines.text + ".txt" //save filepath

      fs.writeFile(newSaveAsFilePath, dataSongLines.wrds, function(err, data) {
        var returnObj = {}
        returnObj.text = newSaveAsFilePath //filepath
        response.writeHead(200, { "Content-Type": MIME_TYPES["json"] });
        response.end(JSON.stringify(returnObj))
      });
